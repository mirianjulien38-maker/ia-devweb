    ...
    <div className="flex-1 h-full bg-slate-900/40 flex flex-col overflow-hidden text-slate-100 font-sans relative">
      ...
      <div className="px-6 py-4 border-b border-slate-800/60 bg-slate-900/50 flex justify-between items-center shrink-0">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-xl bg-gradient-to-tr from-sky-500/10 to-indigo-500/10 border border-sky-500/20 flex items-center justify-center shrink-0">
            <MessageSquare className="w-4 h-4 text-sky-400" />
          </div>
          <div>
            <h4 className="text-white font-bold text-xs uppercase tracking-wider">Mpamorona Chat</h4>
            <p className="text-slate-400 text-xs mt-0.5 font-medium max-w-[250px] sm:max-w-md truncate">
        ...
        {isGenerating && (
          <div className="flex items-center gap-2 bg-sky-500/10 border border-sky-500/20 px-3 py-1.5 rounded-full text-[10px] text-sky-400 font-mono font-bold animate-pulse">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-thin scrollbar-thumb-slate-800">
        {chatHistory.length === 0 ? (
          ...
          <div className="max-w-2xl mx-auto space-y-8 py-6 animate-fadeIn">
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                {PREDEFINED_TEMPLATES.map((template) => (
                  <button
                    ...
                    className="flex text-left p-4 rounded-2xl bg-slate-950/40 hover:bg-slate-950/80 border border-slate-850 hover:border-slate-800 disabled:opacity-50 disabled:cursor-not-allowed transition-all group active:scale-[0.99]"
                  >
                    <div className="bg-slate-900 border border-slate-800 p-2.5 rounded-xl mr-3.5 group-hover:bg-slate-800/80 transition-all shrink-0">
                      {getTemplateIcon(template.icon)}
                    </div>
                    <div className="min-w-0">
                      <h4 className="font-bold text-xs text-slate-200 group-hover:text-white transition-colors">
                        {template.name}
                      </h4>
                      <p className="text-slate-500 text-[10px] mt-1 leading-relaxed font-sans line-clamp-2">
            ...
            <div className="p-4 rounded-2xl bg-sky-500/5 border border-sky-500/10 text-slate-400 text-xs leading-relaxed flex gap-3.5 items-start">
              <Database className="w-5 h-5 text-sky-400 shrink-0 mt-0.5 animate-pulse" />
              <div>
                <p className="font-bold text-slate-200 text-xs mb-1">Fanontaniako momba ny Database (Sequence):</p>
          ...
          <div className="max-w-3xl mx-auto space-y-6">
            {chatHistory.map((msg) => (
              <div
                key={msg.id}
                className={`flex gap-4 ${msg.sender === "user" ? "justify-end animate-slideUp" : "justify-start animate-slideUp"}`}
              >
                {msg.sender === "ai" && (
                ...
                <div
                  className={`flex flex-col rounded-2xl p-4.5 text-xs leading-relaxed ${
                    msg.sender === "user"
                      ? "bg-sky-500 text-white rounded-tr-none shadow-lg shadow-sky-500/10 max-w-[80%]"
                      : "bg-slate-950/60 border border-slate-850/80 text-slate-200 rounded-tl-none max-w-[85%] shadow-xl"
                  }`}
                >
                  ...
                  {msg.code && (
                    <div className="mt-4 pt-4 border-t border-slate-850/60 flex flex-col gap-2.5">
                      ...
                      <div className="flex flex-wrap gap-2">
                        <button
                          ...
                          className="flex-1 min-w-[130px] flex items-center justify-center gap-1.5 bg-sky-500 hover:bg-sky-400 text-white text-[10px] font-extrabold uppercase py-2.5 px-3.5 rounded-xl transition-all shadow-md active:scale-95"
                        ...
                        <button
                          ...
                          className="flex-1 min-w-[130px] flex items-center justify-center gap-1.5 bg-slate-900 hover:bg-slate-855 border border-slate-800 text-slate-300 text-[10px] font-extrabold uppercase py-2.5 px-3.5 rounded-xl transition-all active:scale-95"
                        >
                ...
                {msg.sender === "user" && (
                  <div className="w-9 h-9 rounded-xl bg-slate-850 border border-slate-800 flex items-center justify-center shrink-0 shadow-md">
            ...
            {isGenerating && (
              <div className="flex gap-4 justify-start animate-pulse">
                <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-sky-500 to-indigo-600 flex items-center justify-center shrink-0 shadow-lg shadow-sky-500/15 border border-sky-400/20">
                  <Sparkles className="w-4.5 h-4.5 text-white animate-spin-slow" />
                </div>
                <div className="bg-slate-950/60 border border-slate-850/80 text-slate-200 rounded-2xl rounded-tl-none p-4.5 max-w-[85%] flex flex-col gap-2 shadow-xl">
      ...
      <div className="p-6 border-t border-slate-800/60 bg-slate-950/30 shrink-0">
        <div className="max-w-3xl mx-auto relative">
          <form onSubmit={handlePromptSubmit} className="relative flex items-center">
            <textarea
              ...
              placeholder={
                hasExistingCode
                  ? "Inona no fanitsiana tianao hatao amin'ity tranonkala ity?"
                  : "Soraty eto ny karazana tranonkala tianao hamboarina..."
              }
              className="w-full bg-slate-950/70 border border-slate-850/80 focus:border-sky-500 focus:ring-1 focus:ring-sky-500 text-slate-100 placeholder-slate-700 rounded-2xl px-4 py-4 pr-14 text-xs resize-none h-20 outline-none transition-all shadow-inner leading-relaxed"
              onKeyDown={(e) => {
            ...
            <button
              ...
              className="absolute right-3.5 bottom-3.5 bg-sky-500 hover:bg-sky-400 disabled:bg-slate-900 text-white disabled:text-slate-700 p-2.5 rounded-xl transition-all active:scale-95 shadow-lg"
              title="Alefaso"
            >
              <Send className="w-3.5 h-3.5" />
            </button>
          </form>
          <div className="mt-2.5 flex justify-between text-[10px] text-slate-600 px-1 font-mono select-none">
